...
import de.hdodenhof.circleimageview.CircleImageView1;
...
public abstract class ChatSDKAbstractChatActivity extends ChatSDKBaseActivity implements AbsListView.OnScrollListener {
    ...
    protected void initActionBar() {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.HONEYCOMB) {
            ...
            final CircleImageView1 circleImageView1 = (CircleImageView1) actionBarView.findViewById(R.id.chat_sdk_circle_image);
            ...
            changed = setThreadImage(circleImageView1, imageView, onClickListener) || changed;
     ...
     */
    protected boolean setThreadImage(final CircleImageView1 circleImageView1, final ImageView imageView, final View.OnClickListener onClickListener) {
        final String imageUrl = thread.threadImageUrl();

        if (circleImageView1.getTag() == null || StringUtils.isEmpty(imageUrl) || !imageUrl.equals(circleImageView1.getTag())) {
            if (StringUtils.isEmpty(imageUrl))
                setRoundCornerDefault(circleImageView1, imageView, onClickListener);
            else {
...
//                if (thread.getType()==BThread.Type.Private)
                if (imageUrl.split(",").length > 1) {
                    ...
                    new MakeThreadImage(imageUrl.split(","), size, size, thread.getEntityID(), circleImageView1);
                    circleImageView1.setOnClickListener(onClickListener);
                    circleImageView1.setVisibility(View.VISIBLE);
                    circleImageView1.bringToFront();
                } else
                    VolleyUtils.getImageLoader().get(imageUrl, new ImageLoader.ImageListener() {
                        @Override
                        public void onResponse(ImageLoader.ImageContainer response, boolean isImmediate) {
                            if (response.getBitmap() != null) {
                                circleImageView1.setTag(imageUrl);
                                circleImageView1.setVisibility(View.INVISIBLE);
                                imageView.setVisibility(View.INVISIBLE);
                                circleImageView1.setImageBitmap(response.getBitmap());
                                circleImageView1.setVisibility(View.VISIBLE);
     ...
     */
    protected void setRoundCornerDefault(CircleImageView1 circleImageView1, ImageView roundedCornerImageView, View.OnClickListener onClickListener) {
        circleImageView1.setVisibility(View.INVISIBLE);
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        ...
        if (BDefines.Options.GroupEnabled) {
            ...
            item.setIcon(R.drawable.ic_plus_white);
        }
        ...
        if (BDefines.Options.ThreadDetailsEnabled) {
            ...
            itemThreadUsers.setIcon(R.drawable.ic_info_white);
        }
