...
public class BChatcatNetworkAdapter extends BFirebaseNetworkAdapter {
    ...
    public Promise<BUser, BError, Void> handleFAUser(final FirebaseUser authData) {
        ...
        if (authData == null) {
            ...
            if (getLoginInfo().get(Prefs.TokenKey) != null && token.equals(""))
                token = getLoginInfo().get(Prefs.TokenKey).toString();
    ...
    public Promise<BUser, BError, Void> checkUserAuthenticated() {
        if (DEBUG)
            Timber.v("checkUserAuthenticatedWithCallback, %s", getLoginInfo().get(Prefs.AccountTypeKey));
                    ...
                    if (keys.size() > 0) {
                        ...
                        masterDeferredObject.progress(new ProgressCallback<MasterProgress>() {
                            ...
                            public void onProgress(MasterProgress masterProgress) {
                                ...
                                if (DEBUG)
                                    Timber.d("MasterDeferredProgress, done: %s, failed: %s, total: %s", masterProgress.getDone(), masterProgress.getFail(), masterProgress.getTotal());
        ...
        userToFollowRef.setValue("null", new DatabaseReference.CompletionListener() {
            ...
            public void onComplete(DatabaseError firebaseError, DatabaseReference firebase) {
                if (firebaseError != null) {
                    ...
                    if (DEBUG)
                        Timber.d("followUser, curUserFollowsRef: %s", curUserFollowsRef.toString());
